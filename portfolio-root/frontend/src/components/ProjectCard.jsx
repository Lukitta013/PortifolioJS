...
function ProjectCard({ project }) {
  return (
    <div className="bg-white rounded-2xl shadow-lg overflow-hidden hover:shadow-xl transition-all duration-300 group border border-gray-200">
      {/* Imagem do projeto */}
      ...
      <div className="p-6">
        ...
        <div className="flex flex-wrap gap-1.5 mb-4">
          {project.tecnologias?.split(',').slice(0, 4).map((tech, index) => (
            <span
              key={index}
          ...
          {project.tecnologias?.split(',').length > 4 && (
            <span className="bg-gray-50 text-gray-600 text-xs px-2 py-1 rounded-full border border-gray-200">
              +{project.tecnologias.split(',').length - 4}
            </span>
          )}
        </div>
